...
double Reactietijd[255];
double gemReactietijd = 0;
...
void main_loop()
{
  ...
  if (startState)
  {
    ...
    while (startState && aantalInterval >= counter)
    {
      Blynk.run(); /* keep the Blynk active */
      timer.run(); /* keep the counter for Blynk active */
      if (selectionMade == 0)
      {
        ...
        selectionMade = 1;
        Serial.print("dit is: ");
        Serial.println(selectionLamp);
        counter++;
      }

      if (raakState[selectionLamp] == 0)
      { //niet aangeraakt dan lamp aan, anders lamp uit en nieuwe selectie
        lampState[selectionLamp] = 1;
      }
      else
      {
        Reactietijd[counter] = rt[selectionLamp];
      ...
      if (selectionLamp == 0)
      {
        ...
        if (distance < 10 && selectionLamp == 0)
        {
          ...
          if (timeSet == 1)
          {
            rt[0] = (millis() - bufferTijd) / 1000;
          }
        }
      }
    }
    startState = 0;
    //bereken gemReactietijd
    for (int i = 0; i < counter; i++)
    {
      gemReactietijd = gemReactietijd + Reactietijd[i];
    }
    gemReactietijd = gemReactietijd / counter;
  }
  else
  {
    digitalWrite(D6, HIGH);
...
BLYNK_WRITE(V1) // Read the raak values from Blynk Server
{
  raakState[1] = param.asInt();
}

BLYNK_WRITE(V2) // Read the raak values from Blynk Server
{
  raakState[2] = param.asInt();
}

BLYNK_WRITE(V3) // Read the raak values from Blynk Server
{
  raakState[3] = param.asInt();
}

BLYNK_WRITE(V91) // Read the reactietijd values from Blynk Server
{
  rt[1] = param.asDouble();
}

BLYNK_WRITE(V92) // Read the reactietijd values from Blynk Server
{
  rt[2] = param.asDouble();
}

BLYNK_WRITE(V93) // Read the reactietijd values from Blynk Server
{
  rt[3] = param.asDouble();
}

BLYNK_WRITE(V25) // Read the reactietijd values from Blynk Server
{
  startState = param.asInt();
}

BLYNK_WRITE(V20) // Read the reactietijd values from Blynk Server
{
  aantalInterval = param.asInt();
}
...
void readValue() /* it activates every second*/
{
  ...
  Blynk.virtualWrite(V90, reactieTijd);
  ...
  Blynk.virtualWrite(V25, startState);
  Blynk.virtualWrite(V20, aantalInterval);
}
